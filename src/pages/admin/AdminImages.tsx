...
const AdminImages = () => {
  ...
  const roomImages: Record<string, { src: string; caption: string }[]> = {
    deluxe: [
      { src: roomDeluxe, caption: "Main view" },
      { src: roomSuite, caption: "Interior detail" },
    ],
    suite: [
      { src: roomSuite, caption: "Fireplace view" },
      { src: roomStandard, caption: "Bedroom" },
    ],
    classic: [
      { src: roomStandard, caption: "Room overview" },
    ],
  };

  const heroImages = [
    { src: heroImage, caption: "Mountain sunrise panorama" },
  ];
  ...
  return (
    <div className="space-y-10">
      {/* Room Images */}
      <section>
        <h2 className="font-heading text-xl mb-4">Room Images</h2>
        ...
        <div
          ...
          className="border-2 border-dashed border-border rounded-lg p-8 text-center cursor-pointer hover:border-primary/50 hover:bg-muted/50 transition-colors mb-6"
        >
      ...
      {/* Hero Images */}
      <section>
        <h2 className="font-heading text-xl mb-4">Hero Images</h2>
        ...
        >
          <Upload size={32} className="mx-auto text-muted-foreground mb-2" />
          <p className="text-sm text-muted-foreground">
            Drag & drop hero images here or click to upload
          </p>
        </div>
